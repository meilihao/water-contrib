		...
		AdapterConfig := `
{
    ...
    "Prefix":"cssdb"
		...
		Convey("Basic operations", func() {
			...
			router.Before(cache.New(Adapter, AdapterConfig))
			...
			router.Get("/", func(ctx *water.Context) {
				...
				So(c.Get("400"), ShouldBeNil)
				So(c.Get("uname").(gossdb.Value).String(), ShouldEqual, "unknwon")
		...
		Convey("Increase and decrease operations", func() {
			...
			router.Get("/", func(ctx *water.Context) {
				c := cache.Get(ctx)
				So(c.Incr("404"), ShouldBeNil)
				So(c.Decr("404"), ShouldBeNil)
				...
				So(c.Get("int").(gossdb.Value).Int(), ShouldEqual, 0)
				So(c.Get("int64").(gossdb.Value).Int64(), ShouldEqual, 0)
