...
func TestError(t *testing.T) {
	...
	// code
	router.GET("/server_err", func(c *water.Context) {
		c.String(http.StatusInternalServerError, errors.New("oh no").Error())
	})
...
func TestHTML(t *testing.T) {
	...
	router.GET("/hello", func(c *water.Context) {
		HTML(c, 200, "hello", tmpl, "world")
	})
	...
	for _, s := range spans {
		...
		if s.Name() == "water.renderer.html" {
			tspan = s
			break
		}
	}
	require.NotNil(t, tspan)
	assert.Equal(t, attribute.StringValue("hello"), tspan.Attributes()["go.template"])
}

func TestGetSpanNotInstrumented(t *testing.T) {
	router := water.NewRouter()
	router.GET("/ping", func(c *water.Context) {
